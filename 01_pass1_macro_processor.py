...
def create_ala_entries(ala, arguments):
    ...
    return [{'Index': i + 1, 'Formal Parameter': arg} for i, arg in enumerate(arguments)]
...
def print_table(title, headers, rows):
    ...
    col_widths = [max(len(str(row[h])) for row in rows + [dict(zip(headers, headers))]) + 2 for h in headers]
...
print_table("Macro Definition Table (MDT)", ["Index", "MDT Entry"], [{"Index": i+1, "MDT Entry": entry} for i, entry in enumerate(mdt)])
